...
def draw_result_img(x_test, y_test, x_train, y_train, img_lengths_test, img_uvs_test, type):
    ...
    for img_index in range(len(img_lengths_test)-1):
        ...
        for i in range(len(x_one_img)):
            ...
            if y_one_img[i] == 0:
                cv2.circle(true_img, (col, row), int(radius), [0, 255, 0], -1)
            if y_one_img[i] == 1:
                cv2.circle(true_img, (col, row), int(radius), [0, 255, 255], -1)
            if y_one_img[i] == 2:
                cv2.circle(true_img, (col, row), int(radius), [0, 0, 255], -1)
...
list_train = [1, 2]
x_train_c, y_train_c, img_lengths_train_c, img_uvs_train_c = load_data(list_train, 'c')
x_train_v, y_train_v, img_lengths_train_v, img_uvs_train_v = load_data(list_train, 'v')
x_train_g, y_train_g, img_lengths_train_g, img_uvs_train_g = load_data(list_train, 'g')
...
list_test = [3, 4, 5, 6]
x_test_c, y_test_c, img_lengths_test_c, img_uvs_test_c = load_data(list_test, 'c')
x_test_v, y_test_v, img_lengths_test_v, img_uvs_test_v = load_data(list_test, 'v')
x_test_g, y_test_g, img_lengths_test_g, img_uvs_test_g = load_data(list_test, 'g')
