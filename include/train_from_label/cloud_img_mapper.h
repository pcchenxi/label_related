...
public:
  ...
  {   
    ...
    Mat img_cost  = Mat(image_raw.rows, image_raw.cols, CV_8UC3, Scalar(0));
    ...
    for(int i = 0; i < pcl_cloud_filtered.points.size(); i++)
    {
      ...
      if(uv.x >= 0 && uv.x < image_raw.cols && uv.y >= 0 && uv.y < image_raw.rows)
      {
        ...
        if(depth < depth_before || depth_before == 0)
        {
            ...
            cv::circle(img_cost, uv, 5, Scalar(pcl_cloud_filtered.points[i].b, pcl_cloud_filtered.points[i].g, pcl_cloud_filtered.points[i].r), -1);  
