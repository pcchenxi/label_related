...
void callback_cloud(const sensor_msgs::PointCloud2ConstPtr &cloud_in)
{
    ...
    try 
    {
        tfListener->lookupTransform("world_corrected", "base_link", cloud_in->header.stamp, to_target);
    }
    catch (tf::TransformException& ex) 
    {
    ...
    ground_cloud1_ = cml->process_cloud(pcl_cloud, 16, 16, 6, 0.15, 0.015, robot_x_, robot_y_);
